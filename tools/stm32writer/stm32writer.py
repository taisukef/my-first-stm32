...
try:
	...
	parser.add_option(
		"--no-writing",
		action="store_true",
		dest="no_writing_flag",
		default=False,
		help="no writing to FLASH")
	...
	# boot loader
#	print "try to boot loader!"
	ser = stm32_serial.open_port_with_control(options.port, options.baudrate)
#	ser.rts = False #boot0 = 1
	ser.dtr = True #reset
	time.sleep(0.5)
	ser.dtr = False
...
#	ser = stm32_serial.open_port(options.port, options.baudrate)
	...
	for filename in args:
		...
		# 書き込み
		if not options.no_writing_flag:
